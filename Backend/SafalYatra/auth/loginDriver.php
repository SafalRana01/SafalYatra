...
if (isset(
    ...
    // Prepare SQL query to check if a driver with the provided email exists
    ...
    // If no driver found with that email
    if ($count == 0) {
    ...
    // Get the driver ID from the fetched data
    $driver_id = $driver['driver_id'];

    // Insert the token into the access_tokens table with reference to the driver
    $sql = "INSERT INTO access_tokens (token, driver_id) VALUES('$token', '$driver_id')";
    $result = mysqli_query($connect, $sql);
